...
public class SamplePairs extends PixelBenchmark{
	...
	public double doAnalysis(BufferedImage image, int colour){
		...
		for(starty = 0; starty < imgy; starty++){
			for(startx = 0; startx < imgx-1; startx = startx + 2){
		...
		for(starty = 0; starty < imgy-1; starty = starty + 2){
	...
	public static void main(String[] args){
		if(args.length != 1){
			System.out.println("Usage: RS.SamplePairs <imagefilename>");
			System.exit(1);
		}
		try{
...
//			System.out.println("Result from blue: " + results);
			average += results;
			average = average/3;
			System.out.println("RESULT: " + args[0] + " " + Math.round(100*average));
//			System.out.println();
