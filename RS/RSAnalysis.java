...
public class RSAnalysis extends PixelBenchmark{
	...
	public static void main(String[] args){
		if(args.length != 1){
			System.out.println("Usage: RS.RSAnalysis <imagefilename>");
			System.exit(1);
		}
		try{
			...
			double average = 0;
//			for  (int j=0;j<3;j++){
//			    double[] results = rsa.doAnalysis(image, j, true);
//			    System.out.println("Result from" + j + ": " + results[26] + " (true)");
//			    average += results[26];
//			}
			for  (int j=0;j<3;j++){
			    double[] results = rsa.doAnalysis(image, j, false);
//			    System.out.println("Result from" + j + ": " + results[26] + " (false)");
			    average += results[26];
			}
			System.out.println("Result: " + args[0] + " " + Math.round(100*average/3) + "\n");
		}catch(Exception e){
